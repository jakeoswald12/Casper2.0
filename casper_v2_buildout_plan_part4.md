...
describe('RAG Retrieval System', () => {
  it('should extract keywords from query', () => {
    ...
    expect(keywords).not.toContain('tell');
  });
  ...
  it('should retrieve relevant document chunks', async () => {
    ...
    expect(context.documentChunks).toBeDefined();
    expect(Array.isArray(context.documentChunks)).toBe(true);
  });
});
```
...
library: {
  ...
  addToProject: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Process library book into document chunks
      ...
      const chunks = libraryBook.chapters.flatMap((chapter, chapterIndex) => {
        ...
        return paragraphs.map((paragraph, paragraphIndex) => ({
          ...
          userId: ctx.userId,
          content: paragraph,
          chunkIndex: chapterIndex * 100 + paragraphIndex,
          chunkSize: paragraph.length,
          sectionTitle: chapter.title,
        }));
      });
      
      await ctx.db.insert(documentChunks).values(chunks);
...
- [ ] RAG retrieval returning relevant chunks
...
- [ ] Starred messages in RAG context
